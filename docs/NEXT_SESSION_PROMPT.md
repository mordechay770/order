...
### 0. אבטחה — חובה לפני שימוש אמיתי באדמין
- לקרוא: `docs/SECURITY_NEXT_STEPS.md`
- החלטה מיידית: האם להגן על `admin.html` ב-Vercel Password Protection / פרויקט נפרד / server-side auth
- לא להכניס webhooks אמיתיים או הגדרות רגישות לאדמין ציבורי לפני הגנה ברמת hosting/server
- להחליט האם ממשיכים MVP עם Browser → Make webhook + validation, או מוסיפים Vercel Function דק לפני Make

### 0.5. אישור מנהל ומסך מטבח
- לקרוא: `docs/ADMIN_KITCHEN_WORKFLOW_SPEC.md`
- לפתוח סקיצה ויזואלית: `docs/ops-screen-mockups.html`
- לבנות תחילה אישור מנהל דרך Green API עם כפתורים: אישור, דחייה, לבדיקה
- להוסיף סיכום מוצרים עיקריים למנהל לפני אישור, כדי לזהות בעיות מלאי
- לתכנן שלושה דפים נפרדים: `manager.html` למנהל, `kitchen-board.html` למטבח, `supervisor.html` למשגיח
- לתכנן תמיכה בעברית וברוסית לכל מסכי התפעול
- להוסיף מעקב נפרד לסטטוס הזמנה ולסטטוס תשלום
- להוסיף הרשאות ומשתמשים: אדמין, מנהל, שף, אפייה, סלטים, בישול, משגיח, צפייה בלבד
- לאפשר לאדמין לשלוח הודעות יזומות ללקוח: עיכוב, תזכורת תשלום, בירור, הודעה חופשית
